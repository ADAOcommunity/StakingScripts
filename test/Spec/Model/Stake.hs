...
import Stake (StakeDatum (..), StakeRedeemer (..), stakeScript)

--------------------------------------------------------------------------------

-- | apropos model for testing multisigs.
data StakeModel = StakeModel
  { batchers :: [PubKeyHash]
  -- ^ Batchers allowed to update UTxO(s).
  , datum    :: StakeDatum
  , redeemer :: StakeRedeemer
  -- ^ Datum and Redeemer used.
  , ctx :: ScriptContext
  -- ^ The `ScriptContext` of the transaction.
  }
  deriving stock (Eq, Show)

-- | Propositions that may hold true of a `StakeModel`.
data StakeProp
  = -- | Sufficient number of signatories in the script context.
    UserSigned
  | -- | Insufficient number of signatories in the script context.
    BatchedCorrectly -- This cannot happen currently.
  |
    InvalidTx
    -- | Insufficient signatures or net positive UTxO not present.
  deriving stock (Eq, Show, Ord)

instance Enumerable StakeProp where
  enumerated = [UserSigned, BatchedCorrectly, InvalidTx]

instance LogicalModel StakeProp where
